...
print("1 - Generate (or Re-generate) QR images for ALL guests")
...
# =========================
    ...
    if name.lower() in guests["name"].str.lower().values:
        print("⚠️ This guest already exists.")
    else:
...
# =========================
# MODE 3: ADD MULTIPLE GUESTS
# =========================
elif choice == "3":
    try:
        count = int(input("How many guests to add? "))
        new_entries = []
        ...
        for i in range(count):
            ...
            if m_name.lower() in guests["name"].str.lower().values:
                print(f"⚠️ {m_name} already exists. Skipping.")
                continue
            
            m_uid = str(uuid.uuid4())
            new_entries.append({"name": m_name, "email": m_email, "unique_id": m_uid, "checked_in": "NO"})
        ...
        if new_entries:
            ...
            for entry in new_entries:
                img = qrcode.make(f"{RENDER_URL}/checkin?id={entry['unique_id']}")
                safe_name = re.sub(r"[^\w]", "_", entry['name']).strip()
                img.save(f"qr_codes/{safe_name}.png")
                print(f"✅ QR created for: {entry['name']}")
